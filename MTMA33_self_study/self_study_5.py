...
def assess_flood_risk_perturbed(filename, threshold, lengthts, perturbation):
    '''Assess flood risk in a changed climate.
    Inputs:  name of input data file,  threshold for flood,  array containing the perturbation factors
    Outputs: tuple of the new statistics,  the simulated time series,  the flood risk
    ...
    sim_timeseries = gen_timeseries(new_stats, lengthts)   # Generate a time series based on these statistics
    flood_risk = flood_risk_timeseries(sim_timeseries, threshold)   # Calculate flood risk in the simulated data
    return((new_stats, sim_timeseries, flood_risk))
...
def calculate_statistics(timeseries):
    ...
    Output: An array of length 4 containing the following: [probability of rain given rain the day before,  probability of rain given no rain the day before,  mean rainfall amount,  standard deviation rainfall amount] '''
...
def flood_risk_timeseries(timeseries, threshold=75, days=5):
  ...
  # the number of breaches is the sum of count_floods array.
    return(np.sum(count_floods)/len(timeseries))   # Calculating the probability that a day is a flood 